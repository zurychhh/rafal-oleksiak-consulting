...
(function(){
  ...
  /* Test LITERALNY, nie zakresowy. Pytanie jest jedno: czy liczba perDay stoi
     doslownie w cytacie, ktory model zwrocil. Wykrywanie widelek bylo tu wczesniej
     i mylilo sie — "ok. 300 g dla psa o masie 15-20 kg" ma przedzial, ale opisuje
     on mase psa, a dawka 300 JEST wydrukowana i ma taka zostac.

     Kropke i przecinek zdejmujemy z KONCA i POCZATKU tokenu, nigdy ze srodka:
     poprzednia wersja robila replace(/[.,]/g," ") na calym cytacie, wiec "1,5 miarki"
     rozpadalo sie na "1" i "5" i uczciwy odczyt dostawal falszywy alarm. Przecinek
     dziesietny sprowadzamy do kropki, a porownujemy i tekstem, i liczba, zeby "30"
     zgadzalo sie z 30, a "1.50" z 1.5.                                              */
  ...
  function derived(D){
    ...
    return !quoteHasNumber(D.q,D.perDay);
  }
  ...
  function paint(){
    ...
    $("plot").innerHTML=rows.map(function(F){
      ...
      marks+=m ?'<i class="b2" style="width:'+at(m)+'%"></i>'
               :'<i class="b0" style="width:100%"></i>';
      /* Ten sam test literalny co w panelu i w widzecie: jesli liczby, z ktorej licza sie
         dni zapasu, nie ma doslownie w cytowanych slowach, slupek musi to powiedziec.
         Liczba zostaje, znika tylko jej status odczytu z opakowania.                  */
  ...
  function verdict(F){
    ...
    var off=derived(DOSE[F.k]);
    $("wb").textContent=F._pd+" days at "+DOSE[F.k].perDay+" "+F.unit+" a day"
      +(off?", a figure the pack does not print.":".");
    $("wn").innerHTML="&ldquo;"+esc(DOSE[F.k].q)+"&rdquo; &mdash; the maker."
      +(off?" That daily number does not appear in those words. It was worked out, not read."
  ...
  $("g2").onclick=function(){
    ...
    run($("g2"),$("s2"),function(){
      return SAMPLE.json(
        ...
        +"newlines. Each object: {\"perDay\":number|null,\"doseUnit\":string,\"quote\":string,"
        +"\"note\":string}. perDay = the amount used in ONE DAY, in the unit the "
        +"directions themselves use, and ONLY when the line prints that daily number outright. "
        +"It is a single JSON number, never a range and never a string, and it MUST appear "
        +"verbatim inside the quote you return. Do NOT average. Do NOT multiply a serving by a "
        +"frequency. Do NOT take the middle of a range. If reaching a daily figure would need "
        +"any arithmetic of yours, perDay is null: one scoop of 30 g taken 1-2 times a day is "
        +"null, not 45 and not 1.5. A daily amount that IS printed but carries a condition — so "
        +"much a day for a given body weight, for instance — still counts, because the number "
        +"itself is on the pack: return it. quote = the exact "
        ...
        $("o2").innerHTML=lines.map(function(l,i){
          ...
          /* Sprawdzenie po stronie kodu, bo sam prompt tego nie utrzyma. Liczba liczy
             sie jako ODCZYTANA z opakowania tylko wtedy, gdy stoi doslownie w cytacie.
             Liczby nie ruszamy — zmienia sie wylacznie to, za co ja podajemy.
             Trzy stany, nie dwa: cytat z liczba, cytat bez liczby, brak cytatu.     */
          var onPack=n!==null&&quoteHasNumber(q,n);
          if(F) DOSE[F.k]={perDay:n,q:q.slice(0,80)};
          return row(esc(nm)+(q?' <span style="color:var(--soft)">&mdash; &ldquo;'
              +esc(q.slice(0,52))+'&rdquo;</span>':""),
            onPack?(n+" "+esc(r.doseUnit||"")+"/day")
                  :(q?"not on the pack":"no dose on the label"),
            !onPack);
        }).join("");
        /* Liczymy tylko te, ktore przeszly test literalny. Wczesniej licznik mowil
           "states an amount" takze o liczbie, ktorej na opakowaniu nie ma.          */
        var got=arr.filter(function(r){
          return r&&r.perDay>0&&quoteHasNumber(String(r.quote||""),r.perDay);
        }).length;
        $("s2").textContent=got+" of "+lines.length
          +" labels print a daily amount outright. The rest get no number, and no model "
          +"may work one out.";
